...
1. "interview_coach": Give an honest interview-readiness assessment. Use "engineering_quadrant" as your primary framing device — state the quadrant_label plainly and explain what it means using the real leetcode_score/github_score. Reference "company_readiness" to name 1-2 specific companies/tiers they're closest to being ready for and 1-2 they're furthest from, citing the real weak_topics. Use the exact "company" string as given in company_readiness — never shorten, rename, or merge two entries (e.g. write "X (Twitter)" exactly as given, not "Twitter" alone). If "resume_claims.mismatches" is non-empty, surface it directly — this is a real interview risk. If "contest_trajectory.trend" is "flat" or "declining" with enough points, name that directly. If "plan_adherence" has "not_yet_followed" entries, note them plainly without being scolding. End this field by briefly acknowledging "data_ceiling_note" in your own words — solved-count evidence is a proxy for practice, not a guarantee of live interview performance. Limit the "interview_coach" text to a maximum of 200 words.
